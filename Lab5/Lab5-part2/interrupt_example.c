...
int blink = 1;
int reset = 0;
int interval = 25000000;

/* Stops any existing timer, then enables a new timer with current interval value */
...
volatile int * HPS_timer_ptr = (int *)HPS_TIMER0_BASE; // timer base address
	...
	*(HPS_timer_ptr) = interval; // write to timer load register
...
void pushbutton_ISR( void )
{
	...
	*(KEY_ptr + 3) = press;					// Clear the interrupt
    ...
    // Divide interval by 2 (Rate at which count is incremented doubles)
    // Then, call config_HPS_timer() which will stop the current timer, then restart the timer with the new inerval value
    	interval /= 2;
        config_HPS_timer();
		HEX_bits = 0b00000110;
    }
	else if (press & 0x4) {					// KEY2
    // Multiply interval by 2 (Rate at which count is incremented halves)
    // Then, call config_HPS_timer() which will stop the current timer, then restart the timer with the new inerval value
